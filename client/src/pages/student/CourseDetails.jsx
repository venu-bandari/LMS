...
const CourseDetails = () => {
  ...
  const {allCourses,calculatingRating,calculateChapterTime,calculateCourseDuration
  ...
  return courseData ? (
    ...
    justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      ...
      <div className='max-w-x1 z-10 text-gray-500'>
        <h1 className='md:text-course-details-heading-small-size
        text-course-details-heading-small-line-height
        text-course-details-heading-large-size
        text-course-details-heading-large-line-height
        font-semibold text-gray-800'>{courseData.courseTitle}</h1>
        <p className='pt-4 md:text-base text-sm'
        dangerouslySetInnerHTML={{__html:courseData.courseDescription.slice(0,226)}}></p>

        <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
          <p>{calculatingRating(courseData)}</p>
          <div className='flex'>
          {[...Array(5)].map((_,i)=>(<img key={i} src={i<Math.floor(calculatingRating(courseData))
          ...
          <p className='text-blue-600'>({courseData.courseRatings.length} {courseData.courseRatings.length>1
            ? 'ratings':'rating'})</p>
          <p className='text-gray-500'>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length>1
            ? 'students':'student'}</p> 
        </div>
        <p className='text-sm'>Course by <span className='text-blue-600 underline'>Venu</span></p>
      ...
      md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        ...
        <div className='pt-6'>
          <p className='md:text-x1 text-lg font-medium text-gray-800'>What's in the course?</p>
